const CONFIGS = [
  ...
  { colorB: [0, 255, 100] },   // partículas para o segundo canvas
...
function initCanvasAnimation(canvas, configIndex) {
  ...
  function frame(now) {
    ...
    // fundo com gradiente verde → ciano → azul-esverdeado apenas no segundo canvas
    if (configIndex === 1) {
      const gradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
      gradient.addColorStop(0, "#004D40"); // verde escuro
      gradient.addColorStop(0.5, "#00BFA5"); // verde-ciano
      gradient.addColorStop(1, "#008EAA"); // azul esverdeado
